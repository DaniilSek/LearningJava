...
**TreeMap** - основана на красно-черном дереве, элементы хранятся в отсортированном порядке, каждый новый ключ вставляется в дерево в соответствии с правилом: ключи в левом поддереве меньше текущего, а в правом больше. **TreeMap** не позволяет использовать ключ null (связано с тем, что для сортировки вызываются методы сравнения compareTo или compare и их нельзя применять к null).
```Java
