...
**HashMap**{HashMap} - хэш-таблица/словарь, имеет структуру ключ-значение. Внутри HashMap находится массив корзин (**Buckets**), в каждой корзине хранятся ноды (**Node**), ноды содержат хэш ключа, сам ключ, значение и ссылку на следующую ноду в корзине.
...
```
На основе начального размера и коэффициента загрузки вычисляется порог — максимальное количество элементов, которое можно добавить до начала расширения.  
Пример:  
```
Начальный размер (initial capacity): 16
Коэффициент загрузки (load factor): 0.75
Порог (threshold): 16 * 0.75 = 12
Это значит, что в HashMap можно спокойно добавить 12 элементов. Как только будет попытка добавить 13-й элемент, сработает механизм расширения.
```
