...
-Каждый поток — это отдельная последовательность выполнения (**thread of execution**);
-Все потоки разделяют одну память (**heap**), но имеют собственный стек;
-Управление потоками осуществляет **JVM** и операционная система.
