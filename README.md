...
### [2. Ключевое слово final](#2-что-означает-ключевое-слово-final-перед-объявлением-переменной-или-метода)  
### [3. Перегрузка и переопределение методов](#3-что-такое-перегрузка-overloading-и-переопределение-overriding-приведите-примеры-каждого-случая)  
### [4. Многопоточность](#4-какие-ключевые-особенности-многопоточности-в-java-вам-известны)  
### [5. equals() и hashCode()](#5-опишите-разницу-между-методами-equals-и-hashcode-почему-важно-соблюдать-связь-между-этими-двумя-методами)  
### [6. Collection Framework](#6-перечислите-преимущества-использования-collection-framework-в-java)  
### [7. Обработка исключений](#7-назовите-три-способа-обработки-исключений-в-java)  
### [8. Класс Object](#8-что-такое-класс-object-в-java-и-почему-он-важен)  
### [9. ==, equals() и compareTo()](#9-объясните-разницу-между--equals-и-compareto-методами-в-java-когда-какой-из-них-следует-использовать)  
### [10. Виды исключений](#10-что-означает-термин-исключение-exception-в-java-какие-бывают-виды-исключений-и-какова-разница-между-ними)  
### [11. Сборщик мусора (GC)](#11-опишите-принцип-работы-механизма-сборки-мусора-gc-в-java-какие-существуют-стратегии-gc-и-как-выбрать-подходящую-стратегию-для-приложения)  
### [12. Collection: List и Set](#12-объясните-разницу-между-интерфейсами-collection-list-и-set-приведите-пример-ситуации-когда-лучше-использовать-каждый-из-них)  
### [13. Дженерики (Generics)](#13-что-такое-дженерики-generics-в-java-и-зачем-они-нужны)  
### [14. Наследование и ключевое слово final](#14-расскажите-как-устроено-наследование-в-java-что-значит-ключевое-слово-final-применительно-к-классу-и-методу-можете-привести-пример-переопределения-метода)  
### [15. Создание потоков (Thread vs Runnable)](#15-опишите-принцип-работы-многопоточности-в-java-чем-отличается-создание-потока-через-реализацию-интерфейса-runnable-от-расширения-класса-thread-какие-проблемы-возникают-при-работе-с-потоками-и-как-их-избежать)  
### [16. Сериализация объектов](#16-что-такое-сериализация-объектов-в-java-и-для-чего-она-применяется-какой-интерфейс-реализует-класс-поддерживающий-сериализацию)  
### [17. Аннотации в Java](#17-что-такое-аннотации-в-java-зачем-они-используются-приведите-пару-примеров-стандартных-аннотаций-и-расскажите-как-их-применять)  
...
### [21. Структуры данных](#21-структуры-данных-java)  
### [Практика](#практика-java)  
<br><br> 
...
**Одинаковые хэш-коды НЕ означают равенство** - это математическая неизбежность:  
- Множество хэш-кодов: Обычно это 32-битное целое число (int). Всего существует около 4 миллиардов вариантов.  
- Множество объектов: В теории их бесконечное количество (любая строка, любой сложный объект).

Поскольку объектов гораздо больше, чем возможных хэш-кодов, неизбежно возникают ситуации, когда два разных объекта получают один и тот же хэш-код. Это называется **коллизией**.
```Java
public class Person {
    private String name;
    private int age;
...
```
**Array** - массив значений. Располагается в памяти *последовательно*. Имеет быстрый доступ к значению **O(1)**, т.к есть прямое обращение к ячейке памяти с значением:  
**ячейка_памяти_искомого_элемента = ячейка_памяти_начала_массива + (индекс_значения * размер_ячейки)**  
Вставка элемента в середину происходит за **O(n)**, в конец за **O(1)** - это связано с тем, что вставляя новый элемент, все существующие элементы, стоящие справа, должны будут сдвинуться.
```Java
int[] array = {1, 2, 3};
```
**ArrayList** - динамический список. Похож на **Array**, но умеет расширяться по мере заполнения (по умолчанию 10 элементов, далее увеличивается в 2 раза). Также имеет прямое обращение к ячейке памяти с значением, поэтому у него быстрый доступ к значению **O(1)**, вставка элемента в середину происходит за **O(n)**, в конец за **O(1)**.  
...
list.add("C");
```
**LinkedList** - динамический двусвязный список. Умеет расширяться по мере заполнения, каждый элемент (узел) содержит данные и ссылку (**Link**) на узел, идущий до него, и узел, идущий после него (двусвязный список).  
Элементы *не располгаются в памяти последовательно*, а разбросаны в памяти, из-за этого у **LinkedList** нет прямого обращения к ячейке памяти с значением. Для того, чтобы найти нужный элемент, идет последовательный перебор элементов и просмотр ссылок, на которые они ссылаются, пока не найдется нужный элемент, из-за этого поиск значения идет за **O(n)**. Но вставка элемента происходит быстро, за **O(1)** за счет того, что не нужно сдвигать элементы, нужно просто заменить ссылки, отвечающие за последовательность элементов.  
...
linkedList.add("Z");
```
**HashSet** - самая распространённая и производительная реализация **Set**. Основана на внутреннем механизме [HashMap](#hashmap-section), где элементы выступают в роли ключей (уникальность), а значения игнорируются (у внутренней **HashMap** в поле **value** идет константа **PRESENT**). Порядок элементов хаотичен, не совпадает с порядком добавления.
```Java
...
hashSet.add(30);
```
**LinkedHashSet** - улучшенная версия **HashSet**, которая сохраняет порядок добавления элементов. Основана на механизме [LinkedHashMap](#linkedhashmap-section).
```Java
...
```
**TreeSet** - единственная реализация **Set**, которая сохраняет элементы в отсортированном порядке, внутри имеет механизм [TreeMap](#treemap-section). Основана на красно-чёрном дереве (**RB-tree**), что обеспечивает сбалансированную структуру. Алгоритм красно-черного дерева подразумевает выделение коренвого элемента (**root**) и дальнейшее распределение элементов влево или вправо в зависимости от того больше они корня или меньше. Например вот как будет выглядеть красно-черное дерево, если мы добавим сначала циру **5** (она станет корнем - **root**), а далее добавим цифры **3, 7, 1, 4, 8**:  
```
              (5:B)
            /       \
        (3:R)        (7:B)
...
**B** — чёрный (Black)  
**R** — красный (Red)  
**Красно-чёрное дерево** автоматически перестраивается и перекрашивается, чтобы соблюдать свои свойства:  
1) Чёрная высота вдоль любых путей одинакова.
...
// Вывод: [1, 3, 5]
```
<a name="hashmap-section"></a>
**HashMap** - хэш-таблица/словарь, имеет структуру ключ-значение. Внутри HashMap находится массив корзин (**Buckets**), в каждой корзине хранятся ноды (**Node**), ноды содержат хэш ключа, сам ключ, значение и ссылку на следующую ноду в корзине.
Если не задать размер вручную, в **HashMap** будет 16 корзин (**DEFAULT_INITIAL_CAPACITY**). Размер массива корзин должен быть степенью двойки
```Java
HashMap<String, Integer> map = new HashMap<>(32); // Начальный размер 32 корзины
```
Элементы распределяются по корзинам с помощью алгоритма расчета индекса корзины, в котором участвует хэш ключа добавляемого элемента.  
**H** = ```(h) ⊕ (h>>>16)```  
```h``` — исходный хеш-код  
```>>>``` — беззнаковое сдвиг вправо (на 16 бит)  
```⊕``` — побитовое XOR (исключающее ИЛИ)  
**bucket_index** = ```H & (capacity − 1)```  
```H``` — финальный хеш-код после сдвига  
```capacity``` — текущий размер массива корзин (всегда степень двойки)  
```&``` — побитовое И  
После выполнения данных операций элемент попадает в корзину по вычесленному индексу.  
Например, пусть хэш ключа элемента ```h = 123456789```, тогда вычисление индекса будет выглядеть так:  
```
H = 123456789 ⊕ (123456789>>>16)  
H = 123456789 ⊕ 1872077  
H = 121584712  
bucket_index = 121584712 & (16−1)  
bucket_index = 121584712 & 15  
bucket_index = 8
```
Пример внутренней структуры **HashMap**:
```
HashMap
└── Массив корзин (Bucket[])
    ├── Bucket[0]
    ├── Bucket[1]
    │   └── Node(key=1, value=X)
    ├── Bucket[2]
    ...
    └── Bucket[16]
```
Если в HashMap становится **64** корзины и одна из них содержит **8** элементов, то структура данной корзины изментися с двусвязного списка но красно-черное дерево для ускорения поиска с **O(n)** до **O(log(n))**. Если элементов в корзине становится меньше **6**, то структура возвращается к списку.  
У **HashMap** есть механизм расширения, по умолчанию коэффициент загрузки (**Load Factor**) равен 0.75 (75%).  
```Java
HashMap(int initialCapacity, float loadFactor)
```
На основе начального размера (**initialCapacity**) и коэффициента загрузки (**loadFactor**) вычисляется порог — максимальное количество элементов, которое можно добавить до начала расширения.  
Пример:  
```
Начальный размер (initial capacity): 16
Коэффициент загрузки (load factor): 0.75
Порог (threshold): 16 * 0.75 = 12
Это значит, что в HashMap можно спокойно добавить 12 элементов.
Как только будет попытка добавить 13-й элемент, сработает механизм расширения.
```
Расширение увеличивает количество корзин в два раза: ```16 -> 32 -> 64...``` А для каждого из элементов заново вычисляется индекс корзины.  
**Коллизия** - это ситуация, когда два ключа попадают в одну корзину.  
Коллизии бывают двух видов:  
1) По хэшу ключа - когда хэш ключа у элементов одинаковый, они попадают в одну корзину, далее идет сравнение ключей через ```equals()```, если ключи разные, то оба элемента остаются, если одинаковые, то старое значение перезаписывается;
2) По индексу корзины - когда хэши ключей разные, но вычисление индекса корзины приводит элементы в одну и ту же корзину.
...
hashMap.put("three", 3);
```
<a name="linkedhashmap-section"></a>
**LinkedHashMap** - все то же самое, что и у **HashMap**, но у узлов (**Node**) появились дополнительные указатели на другие ноды, позволяющие запоминать порядок вставки. Например элемент **B**, попавший в корзину **3**, будет знать, что до него был вставлен элемент **A**, находящийся в корзине **1**.
```Java
...
```
<a name="treemap-section"></a>
**TreeMap** - основана на красно-черном дереве, элементы хранятся в отсортированном порядке, каждый новый ключ вставляется в дерево в соответствии с правилом: ключи в левом поддереве меньше текущего, а в правом больше. **TreeMap** не позволяет использовать ключ null, если не предоставлен компаратор, который умеет с ним работать (связано с тем, что для сортировки вызываются методы сравнения compareTo или compare и их нельзя применять к null).
```Java
