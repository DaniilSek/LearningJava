...
list.add("C");
```
**LinkedArrayList** - динамический двусвязный список. Умеет расширяться по мере заполнения, каждый элемент содержит ссылку (*Link*) на элемент, идущий до него, и идущий после него (двусвязный список).  
...
// Вывод: [1, 3, 5]
```
HasMap - хэш-таблица/словарь, имеет структуру ключ-значение. Внутри HashMap находится массив корзин, в каждой корзине хранятся ноды (Node), ноды содержат хэш ключа, сам ключ, значение, ссылку на следующую ноду в корзине.
```Java
