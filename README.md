...
```
**TreeSet** - единственная реализация **Set**, которая сохраняет элементы в отсортированном порядке. Основана на красно-чёрном дереве (**RB-tree**), что обеспечивает сбалансированную структуру. Алгоритм красно-черного дерева подразумевает выделение коренвого элемента (**root**) и дальнейшее распределение элементов влево или вправо в зависимости от того больше они корня или меньше. Например вот как будет выглядеть красно-черное дерево, если мы добавим сначала циру **5** (она станет корнем - **root**), а далее добавим цифры **3, 7, 1, 4, 8**:  
```
              (5:B)
            /       \
        (3:R)        (7:B)
...
**LinkedHashMap** - все то же самое, что и у **HashMap**, но у узлов (**Node**) появились дополнительные указатели на другие ноды, позволяющие запоминать порядок вставки. Например элемент **B**, попавший в корзину **3**, будет знать, что до него был вставлен элемент **A**, ноходящийся в корзине **1**.
```Java
